...
> Each phase row links to the headline commit on `staging`.

## All 9 phases — v0 LIVE on `staging`
...
| 3 | Unified `<StreamList>` | ✓ v0 | `d92cae9` (homepage mode-tab switch; /events + /issues page migration deferred to v1) |
...
| 6 | `/intro` = photo-driven chapters + HomeClient.js retired | ✓ v0 | `3d66f5b` |
| 7 | `/discussions` + `/members/[slug]` scaffold | ✓ v0 (read-only) | `5a25a82` (3 routes + mock + translations + CSS) |
...
- **/intro.** Two photo-driven chapters: "Five steps, one journey" (sourced from the existing `coreIdea` block — single source of truth) + "Community runs it" (three new tiles linking to /ledger and /learn). Fixed right-rail jump-nav at ≥ 1180px. Brochure cheese stripped; HomeClient.js deleted.
...
- **Phase 3 v1** — `<StreamList>` body migration of `/events` + `/issues` pages (interconnected list/preview/map/filter logic; isolated commit later).
- **Phase 4 v1** — wire the disabled view-switch on /events (List ↔ Thumbnails ↔ Map layouts).
- **Phase 6 v1** — `siteContent.js` prune of HomeClient-exclusive copy blocks (volunteerInvite / cleanupAreas / buildInPublic / heroPanel) + `home.css` per-selector prune of HomeClient-exclusive rules (`.workflow-*` / `.cleanup-*` / `.volunteer-*` etc.) + "What is Shramdan?" discreet link in homepage brand block.
- **Phase 7 v1** — backend write paths: post topic / post message / cast vote / withdraw vote / anonymity checkbox in composer / discussion-presence label on event detail pages / notifications on proposal promotion.
...
- Commits landed on `staging`: 30+ across the session (initial pivot ADR + plan, principles, Phase 1, full Phase 2 v0-v4, real search, Phase 3 v0, Phase 4 fix, Phase 5 batch, Phase 6 v0, Phase 7 v0, Phase 8 specs, Phase 9 a11y, cleanup pass).
- Branches: everything on `staging` per the project's branch workflow. Origin push deferred to a manual end-of-day sweep.
