...
- [ ] P1 [from 14] **Retire / re-home the standalone `/app-development` App Dev Tasks board** now that the nav routes to `/discussions`. ⚠️ NOT a throwaway intro — it is a full functional board (5-stage pipeline proposed→discussion→accepted→in_progress→shipped, vote, take-lead, skill tags, leader assignment, `/app-development/new`, `/app-development/[slug]`, `appDevStub.js`, `docs/api-requirements/app-development.md`). `/discussions` does **not** currently replicate those task-board features. Decide: (a) delete the route + stub + contract, (b) `redirect /app-development → /discussions` (reversible, keeps code), or (c) fold the task-board features into `/discussions` first, then delete. Confirm with the product owner before removing — effort:M
