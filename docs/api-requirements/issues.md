...
**Coordinator ≡ Leader — _frontend convention, flagged for backend_ (2026-06-22):**

- The FE now treats the **`COORDINATOR` eventRole as the same thing as leadership** and shows **only one** seat for it: a dedicated Coordinator slot (issue → `WANT_TO_LEAD` vote; event → the resolved `eventLeader`). `COORDINATOR` is therefore **dropped from the participation-role grid** on both surfaces, and any `rolePlan` `COORDINATOR` entry on an event is not rendered as a separate joinable role. Backend still models them separately (`eventRole = COORDINATOR` vs `WANT_TO_LEAD` / `eventLeaderId`). **Note for backend:** consider collapsing `COORDINATOR` into the leadership concept (or confirm they should stay distinct) — if a `rolePlan` ever needs multiple coordinators *as well as* a single leader, this FE merge would need revisiting. Low-stakes today; logged so the model divergence is tracked.
