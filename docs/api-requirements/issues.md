...
- **`PATCH /issues/{id}` by a non-reporter crashes with `500 UNKNOWN_ERROR` (found 2026-07-02).** Any PATCH (even a no-op `addressText`) by anyone other than the original reporter — **including ADMIN** — returns `500 {"errorCode":"UNKNOWN_ERROR"}` instead of a `403`. Verified live on `api.shramdan.org` with the admin account against issues reported by seed users; the same PATCH on an admin-reported issue succeeds. Two asks: **(a)** return a proper `403` (or better, **let ADMIN edit any issue** — the control-center edit form `/admin/issues/[id]/edit` calls exactly this endpoint and is currently broken for every issue the admin didn't author); **(b)** see the dead-cover data fix below, which this bug blocks.

- **Data fix: 20 live issues still carry covers on the dead `cdn.shramdan.org` host (2026-07-02).** Old seed batches (`open15-*`, `seed-issue-cover-*`) wrote raw cover URLs on a host that no longer resolves — every page view logs `ERR_NAME_NOT_RESOLVED` and the cards render without art. We repaired the 2 admin-reported ones via the real upload pipeline (`scripts/fix-dead-cdn-covers.mjs` — presign → R2 PUT → confirm → PATCH `coverImageId`); the other 20 are blocked by the 500 above because their reporters are seed accounts we can't log in as. **Ask:** either fix the covers DB-side (any real R2 upload URL will do; repeats are fine) or make PATCH admin-capable and we re-run the script. Affected issue ids: `f481ec25`, `dcd82cf8`, `4a600d08`, `407b340d`, `7a4fc950`, `eea9ede7`, `755d3878`, `a540a6f3`, `90c8a1c6`, `7a152e18`, `3ef48f95`, `41287499`, `ffbccfad`, `8d3e7284`, `a575c191`, `f53ebf32`, `0abd3a1f`, `2b05299a`, `193646fc`, `9c1ab038` (full UUIDs in the script's failure log / `GET /campaigns` filter on `cdn.shramdan.org`). Frontend meanwhile substitutes a category-matched local demo photo for dead-host covers (`resolveUsableImage` in `adminUtils`), so no broken requests fire — remove that shim once the data is clean.

---

## Recent changes

- `2026-07-02` — **Dead-CDN cover cleanup.** Fixed the 2 admin-reported issues via `scripts/fix-dead-cdn-covers.mjs`; logged the `PATCH /issues/{id}` non-reporter `500` bug + the 20-issue DB-side cover ask (see Gaps). FE ships a dead-host → demo-photo substitution (`resolveUsableImage`) across the campaign feed, events list and campaign detail hero until the data is repaired.
