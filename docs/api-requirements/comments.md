...
- `2026-06-17` — **Reporting wired; replies endpoint surfaced.** The comment flag now hits the real `POST /comments/{id}/report` (`reportCommentRemote`), with reasons aligned to the backend enum (`SPAM | ABUSE | HARASSMENT | MISINFORMATION | INAPPROPRIATE | OTHER`, optional `details`); 409 = already reported, treated as success, and the local overlay is kept for the per-viewer "Reported" state + auto-hide threshold. `GET /comments/{id}/replies` (cursor) is now wrapped as `fetchCommentReplies` but not yet consumed — the section still loads the whole tree via `fetchComments?limit=200`, so it's a future lazy-load hook for deep threads.
