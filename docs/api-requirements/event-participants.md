...
`EVENT_SCHEDULED`) → Join-as-Role / Lead · event `SCHEDULED` → Join-as-Worker only ·
event `ACTIVE` → Join-as-Worker only · `COMPLETED`/`PAUSED`/`CANCELLED` → none.
(**SCHEDULED was narrowed from all-roles to WORKER-only** per the 2026-06-25
campaign-unification decision; `PAUSED` is a no-join state. See Recent changes
2026-06-25.) Backend needs fall out of this:
...
- **Enforce `SCHEDULED` = WORKER-only (hardening, requested 2026-06-25).** `POST /events/{id}/participants` should reject non-`WORKER` participation roles while the event is `SCHEDULED`, matching the `ACTIVE` rule. The live OpenAPI description currently accepts any role at `SCHEDULED`; the frontend already gates the join modal to `WORKER` (`eventJoinPhase` in `src/lib/issueActions.js`), so this is a defense-in-depth / consistency ask, **not** a UX blocker — a direct API caller could still attach a non-WORKER role at `SCHEDULED` today. Confirms the 2026-06-25 product decision that `SCHEDULED` is Worker-only (not advance multi-role signup). **To verify (backend currently unreachable):** re-read the `POST /events/{id}/participants` description on the next API refresh — if it already restricts SCHEDULED to WORKER, this ask is already done.

---

## Recent changes

- `2026-06-26` — **✅ RESOLVED — backend now enforces `SCHEDULED` = WORKER-only.** The refreshed live spec (`backend.shramdan.org/api-docs.json`) describes `POST /events/{id}/participants` as: `DRAFT → any role · SCHEDULED → WORKER only; others rejected · ACTIVE → WORKER only · PAUSED/COMPLETED/CANCELLED → closed`. This is the exact campaign model (incl. `PAUSED` no-join), so the FE gate (`eventJoinPhase`) and the backend now agree end-to-end. Behavioural POST-test not run (staging DB was empty — 0 events), but the deployed contract description is unambiguous. The 2026-06-25 ask below is now satisfied.
- `2026-06-25` — **Gating decision: `SCHEDULED` is WORKER-only; `PAUSED` is no-join.** The campaign-unification pass locked the lifecycle join rule to: `OPEN`/`DRAFT` = all roles · `SCHEDULED`/`ACTIVE` = WORKER only · `COMPLETED`/`PAUSED` = no join. The frontend `eventJoinPhase` (`src/lib/issueActions.js`) already enforced WORKER-only at `SCHEDULED`, and now also returns `{ joinable: false, roleScope: [] }` for `PAUSED` (it was previously falling through to the all-roles default — a bug). **Backend ask:** tighten `POST /events/{id}/participants` to reject non-WORKER at `SCHEDULED` (see Requested capabilities → "Enforce SCHEDULED = WORKER-only"). This is hardening, not a UX blocker (FE already gates it). Source: `docs/superpowers/specs/2026-06-25-campaign-unification-design.md`. **→ resolved 2026-06-26, see above.**
