...
- **role** (`enum`, required, public) — the lane being applied for. **As of 2026-06-05, the frontend collapses to three top-level lanes** that mirror `docs/product/roles.md` (2026-06-03 restructure): `EVENT_PARTICIPATION`, `DEVELOPMENT`, `COMPANY_MANAGEMENT`. The multi-step `/join` form sends one of these three values directly. The previous flat list (`FRONTEND_DEVELOPER`, `BACKEND_DEVELOPER`, `UI_UX_DESIGNER`, `GRAPHICS_DESIGNER`, `LEGAL`, `FINANCE`, `DONOR`, `COMMUNITY_MANAGER`, `VOLUNTEER`, `OTHER`, plus the UI-only drift `QA_ENGINEER`, `DEVOPS_ENGINEER`, `CONTENT_WRITER`, `TRANSLATOR`, `PHOTOGRAPHER`, `LIVESTREAMER`) is **deprecated as the surface value** but should remain in the backend enum during the transition so legacy stored records do not break. The three new lane values must be **added to the backend enum** for the multi-step form to land without 400s — this is the next reconciliation step.
...
- `2026-06-05` (multi-step UI refactor) — `/join` rebuilt as a five-step centered flow (intro → basics → role → work → motivation). The `role` field collapses to three lane buckets (`EVENT_PARTICIPATION`, `DEVELOPMENT`, `COMPANY_MANAGEMENT`) — see the role bullet under *Fields*. The work step (portfolio, resume, experience) is **skipped when role = `EVENT_PARTICIPATION`**, so submissions under that lane will carry no `resumeId` and no `portfolio`. The `additionalInfo` field was removed from the UI; submissions send the literal `"n/a"` until the backend marks the field optional. Backend action items: (1) add the three new lane enum values to the closed enum; (2) accept (or ignore) `"n/a"` in `additionalInfo`; (3) keep the legacy flat role enum entries for stored historical records during the transition.
