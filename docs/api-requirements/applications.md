...
- `2026-06-17` — **Application submission now creates a verified account.** `POST /applications` requires `otp` + `password` and returns `{ user, application, accessToken, refreshToken }` (201) — it replaced the retired `/auth/register` + `/auth/verify-otp` SMS flow as the member-signup path. New `POST /applications/request-otp { email }` emails a 6-digit code first (409 if the email already has an account, 429 on cooldown). `/join` gained a 5th "verify" step (password + confirm + OTP) and now signs the applicant in on success; the lightweight `/signup` page runs the same endpoints with a default `role: VOLUNTEER` + default motivation. The frontend `applicationRoles` list still carries `QA_ENGINEER`/`DEVOPS_ENGINEER`/`CONTENT_WRITER`, which are **not** in the live enum — harmless today since both forms hard-code `VOLUNTEER` (see [`../engineering/09-backend-admin-gaps.md`](../engineering/09-backend-admin-gaps.md)).
