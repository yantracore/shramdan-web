...
- **sort** (`enum`) — one of `participantCount`, `createdAt`, `scheduledAt`. Mirrors the `sort` param on `GET /issues` so the two list pages can share filter UI.
- **order** (`enum`) — `asc` or `desc`. Applied to `sort` plus the `createdAt` tiebreaker.
