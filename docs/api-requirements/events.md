...
- `2026-06-23` — **Filter-parity ask RESOLVED.** Live spec confirms `GET /events` now accepts `category`, `municipality`, `ward`, `provinceId`, `districtId`, `minVoteCount`, `sort`, `order`, the `north/south/east/west` bounding box, plus `fromDate/toDate/search/limit/cursor` — the full canonical shape in the Filters section is server-side. The 2026-06-05 "filters fall back to client-side" entry below is now historical. Remaining open events asks consolidated in [`00-OUTSTANDING.md`](00-OUTSTANDING.md): `viewerParticipation` echo, embedded issue `translations`, activate/reminder-cadence/pause-resume operations, risk-level SSE.
