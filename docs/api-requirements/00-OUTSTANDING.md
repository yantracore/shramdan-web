...
2. **issues** — expose the scheduled event on `GET /issues/{id}`: embed `event { id, slug, status, scheduledAt, leaderId, viewerParticipation }` on `EVENT_SCHEDULED` issues. A `GET /events?issueId=` filter is an acceptable fallback. FE band-aid (`resolveEventForIssue`) costs an extra `/events` fetch per promoted-issue view. **Was the hard blocker for the unified `/campaign/[id]` detail merge (Phase 4).** ✅ **RESOLVED — live-verified 2026-06-26 on `api.shramdan.org`.** `GET /issues/{id}` now embeds `event { id, slug, status, scheduledAt, leaderId }` on promoted issues — observed directly on multiple promoted issues (both `EVENT_DRAFT` and `COMPLETED`). The FE already consumes it via `getIssueEventId(issue)`, so the district-match workaround can be retired and Phase 4 is unblocked. (The OpenAPI spec still doesn't document the response schema — confirmed by live observation, not the spec.) → [issues.md](issues.md), [campaign-unification spec](../superpowers/specs/2026-06-25-campaign-unification-design.md)
