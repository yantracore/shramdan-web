...
async function countStatus(status, { provinceId, districtId }) {
  const path = status === "OPEN" ? "/issues" : "/events";
  const response = await getJson(path, {
    params: { status, limit: COUNT_LIMIT, provinceId, districtId }
  });
  return getListItems(response).length;
}

export function useCampaignCounts({ provinceId, districtId }) {
  const [counts, setCounts] = useState({});
  const [loading, setLoading] = useState(true);

  const load = useCallback(async () => {
    const settled = await Promise.allSettled(
      CAMPAIGN_STATUS_SEQUENCE.map((s) => countStatus(s, { provinceId, districtId }))
    );
    const next = {};
